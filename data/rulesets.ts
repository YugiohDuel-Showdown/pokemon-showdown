...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	datamod: {
		...
		onSwitchIn(pokemon) {
			...
			if (pokemon.illusion) {
				...
				if (this.effectState[`${species.name}`].illusion.switchedIn) return;
				this.effectState[`${species.name}`].illusion.switchedIn = true;
			} else {
				this.add('-start', pokemon, 'typechange', pokemon.getTypes(true).join('/'), '[silent]');
				if (this.effectState[`${species.name}`].switchedIn) return;
				this.effectState[`${species.name}`].switchedIn = true;
			}
		...
		onDamagingHit(damage, target, source, move) {
			if (target.hasAbility('illusion')) {
				...
				if (!this.effectState[`${target.species.name}`].switchedIn) {
					this.effectState[`${target.species.name}`].switchedIn = true;
	...
	flatrules: {
		effectType: "ValidatorRule",
		name: "Flat Rules",
		desc: "The in-game Flat Rules: Adjust Level Down 50, Species Clause, Item Clause = 1, -Mytical, -Restricted Legendary, Bring 6 Pick 3-6 Depending on game type.",
	...
	oneboostpasserclause: {
		...
		onValidateTeam(team) {
			const boostingEffects = [
				'acidarmor', 'agility', 'amnesia', 'apicotberry', 'barrier', 'bellydrum', 'bulkup', 'calmmind', 'cosmicpower', 'defensemode', 'curse',
	...
	batonpassstatclause: {
		...
		onValidateTeam(team) {
			const boostingEffects = [
				...
				'barrier', 'bellydrum', 'bulkup', 'calmmind', 'cellbattery', 'charge', 'chargebeam', 'coil', 'cosmicpower', 'defensemode', 'cottonguard', 'curse',
